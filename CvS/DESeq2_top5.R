...
# ── LOAD DATA ─────────────────────────────────────────────────────────────────
countsdata <- read.delim("Brassica_susceptible_counts.txt", row.names = "gene_id")
...
susceptible_samples <- rownames(coldata[coldata$condition == "susceptible", ])
...
top5_up$mean_susceptible <- rowMeans(top5_up_counts[, susceptible_samples])
...
top5_down$mean_susceptible <- rowMeans(top5_down_counts[, susceptible_samples])
...
pheatmap(assay(ntd)[rownames(top5_up_final), ],
         ...
         cluster_cols   = TRUE,
         annotation_col = df,
         main           = "Control vs Susceptible — Top 5 Upregulated Genes (padj < 0.05)")
...
pheatmap(assay(ntd)[rownames(top5_down_final), ],
         ...
         cluster_cols   = TRUE,
         annotation_col = df,
         main           = "Control vs Susceptible — Top 5 Downregulated Genes (padj < 0.05)")
